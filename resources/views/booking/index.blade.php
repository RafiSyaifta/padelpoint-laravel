            ...
            <div class="space-y-6">
                ...
                @else
                    @foreach ($bookings as $booking)
                        <div class="bg-white rounded-[1.5rem] md:rounded-[2rem] shadow-sm border border-gray-100 overflow-hidden hover:shadow-xl hover:border-indigo-100 transition-all duration-300 flex flex-col md:flex-row group">
                            <div class="md:w-2/3 p-6 md:p-8 flex items-start gap-4 md:gap-6">
                                <div class="h-12 w-12 md:h-16 md:w-16 bg-indigo-50/70 text-indigo-600 rounded-xl md:rounded-2xl flex items-center justify-center shrink-0 group-hover:bg-indigo-100 transition-colors duration-300">
                                ...
                                <div class="flex-1 min-w-0">
                                    @if ($booking->status == 'success')
                                        <span class="inline-block py-1 px-3 rounded-full bg-green-50 border border-green-100 text-[9px] md:text-[10px] font-black tracking-wider uppercase mb-2 text-green-600">Lunas / Sukses</span>
                                    @elseif($booking->status == 'pending')
                                        <span class="inline-block py-1 px-3 rounded-full bg-orange-50 border border-orange-100 text-[9px] md:text-[10px] font-black tracking-wider uppercase mb-2 text-orange-600">Menunggu Pembayaran</span>
                                    @else
                                        <span class="inline-block py-1 px-3 rounded-full bg-red-50 border border-red-100 text-[9px] md:text-[10px] font-black tracking-wider uppercase mb-2 text-red-600">Dibatalkan</span>
                                    @endif

                                    <h4 class="text-xl md:text-2xl font-black text-gray-900 mb-2 group-hover:text-indigo-700 transition-colors duration-300 truncate">
                                        {{ $booking->court->name }}</h4>

                                    <div class="flex flex-col sm:flex-row sm:items-center gap-2 md:gap-4 text-gray-500 font-medium text-xs md:text-sm mb-4">
                                        ...
                                        <div class="flex items-center gap-1.5 px-2 py-0.5 bg-gray-50 rounded-lg text-gray-700 w-fit">
                                            <svg class="w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    ...
                                    <!-- Equipment & Buttons (Mobile Optimized) -->
                                    <div class="flex flex-col gap-4">
                                        @if($booking->equipment->count() > 0)
                                        <div class="flex flex-wrap gap-2">
                                            @foreach($booking->equipment as $item)
                                                <div class="flex items-center gap-1.5 px-2 py-1 bg-indigo-50/50 rounded-lg border border-indigo-100/50">
                                                    <span class="text-indigo-600 font-black text-[10px]">{{ $item->pivot->quantity }}x</span>
                                                    <span class="text-indigo-700 text-[9px] font-bold uppercase tracking-widest">{{ $item->name }}</span>
                                                </div>
                                            @endforeach
                                        </div>
                                        @endif
                                        ...
                                        @if ($booking->status == 'success')
                                            <button onclick="document.getElementById('review-modal-{{ $booking->id }}').classList.remove('hidden')"
                                                class="w-fit inline-flex items-center gap-2 px-4 py-2 bg-indigo-50 text-indigo-600 rounded-xl font-black text-[9px] uppercase tracking-widest hover:bg-indigo-600 hover:text-white transition-all duration-300 border border-indigo-100">
                            ...
                            <div class="md:w-1/3 bg-gray-50/50 group-hover:bg-gray-50 p-8 border-t md:border-t-0 md:border-l border-dashed border-gray-200 transition-colors duration-300 flex flex-col justify-between items-start md:items-end text-left md:text-right relative">
                                ...
                                @if ($booking->status == 'pending')
                                    <form id="cancel-form-{{ $booking->id }}" action="{{ route('booking.destroy', $booking->id) }}" method="POST" class="w-full mt-auto">
                                        @csrf
                                        @method('DELETE')
